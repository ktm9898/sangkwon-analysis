...
2. **미션**: 지도 이미지에서 "${keyword}" 업종에 해당하는 모든 매장의 **심볼(아이콘) 혹은 마커의 정중앙 위치**를 찾아내세요. 글자(상호명)는 매장 확인용으로 사용하되, 좌표는 반드시 아이콘의 중심을 기준으로 합니다.
3. **위치 좌표**: 아이콘 중앙의 가로(x), 세로(y) 좌표를 0에서 1000 사이의 숫자로 응답하세요. 
